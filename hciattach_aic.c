...
static int read_addr_from_file(const char *path, char *bdaddr)
{
	if (!path)
		return -1;
	...
	int len = sizeof("xx:xx:xx:xx:xx:xx");
	...
	if (sscanf((const char *)buffer, "%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx%1c",
			...
			&local_addr[4], &local_addr[5], &trailing_char) < 6) {
		close(fd);
		return -1;
	}

	memcpy(bdaddr, local_addr, 6);
	close(fd);
	return 0;
}
...
int aic_config_init(int fd, struct uart_t *u, struct termios *ti)
{
	...
	read_addr_from_file("/sys/class/addr_mgt/addr_bt", local_bdaddr);
